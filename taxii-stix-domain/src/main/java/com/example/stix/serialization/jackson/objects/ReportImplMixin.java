...
@JsonPropertyOrder({TYPE_PROPERTY, ID_PROPERTY, CREATED_BY_REF_PROPERTY, CREATED_PROPERTY, MODIFIED_PROPERTY,
        REVOKED_PROPERTY, Report.LABELS_PROPERTY, EXTERNAL_REFERENCES_PROPERTY, OBJECT_MARKING_REFS_PROPERTY,
        GRANULAR_MARKINGS_PROPERTY, NAME_PROPERTY, DESCRIPTION_PROPERTY, PUBLISHED_PROPERTY, OBJECT_REFS_PROPERTY})
public abstract class ReportImplMixin {
    ...
    @JsonProperty(Report.LABELS_PROPERTY) abstract List<String> getLabels();
    ...
    @JsonProperty(PUBLISHED_PROPERTY) abstract ZonedDateTime getPublished();

    @JsonProperty(OBJECT_REFS_PROPERTY) abstract List<Identifier> getObjectRefs();

    @JsonProperty(DESCRIPTION_PROPERTY) abstract String getDescription();

    @JsonCreator
    ReportImplMixin(@JsonProperty(value = TYPE_PROPERTY, required = true) StixType type,
            ...
            @JsonProperty(value = Report.LABELS_PROPERTY, required = true) List<String> labels,
            @JsonProperty(value = NAME_PROPERTY, required = true) String name,
            @JsonProperty(value = PUBLISHED_PROPERTY, required = true) ZonedDateTime published,
            @JsonProperty(value = OBJECT_REFS_PROPERTY, required = true) List<Identifier> objectRefs,

...
@JsonPropertyOrder({TYPE_PROPERTY, ID_PROPERTY, CREATED_BY_REF_PROPERTY, CREATED_PROPERTY, MODIFIED_PROPERTY,
        ...
        GRANULAR_MARKINGS_PROPERTY, FIRST_OBSERVED_PROPERTY, LAST_OBSERVED_PROPERTY, NUMBER_OBSERVED_PROPERTY,
        OBJECTS_PROPERTY})
public abstract class ObservedDataImplMixin {
    ...
    @JsonProperty(FIRST_OBSERVED_PROPERTY) abstract ZonedDateTime getFirstObserved();

    @JsonProperty(LAST_OBSERVED_PROPERTY) abstract ZonedDateTime getLastObserved();

    @JsonProperty(NUMBER_OBSERVED_PROPERTY) abstract int getNumberObserved();

    @JsonProperty(OBJECTS_PROPERTY) abstract ObservableObjects getObjects();

    @JsonCreator
    ObservedDataImplMixin(@JsonProperty(value = TYPE_PROPERTY, required = true) StixType type,
            ...
            @JsonProperty(value = FIRST_OBSERVED_PROPERTY, required = true) ZonedDateTime firstObserved,
            @JsonProperty(value = NUMBER_OBSERVED_PROPERTY, required = true) int numberObserved,
            @JsonProperty(value = LAST_OBSERVED_PROPERTY, required = true) ZonedDateTime lastObserved,
            @JsonProperty(value = OBJECTS_PROPERTY, required = true) ObservableObjects observableObjects) {
